import React from 'react';
import { Link, useNavigate, useLocation } from 'react-router-dom';
import { useAuth } from '../context/AuthContext'; // Hook
import { 
  FaFilm, FaCalendarAlt, FaTools, FaUsers, 
  FaSignOutAlt, FaUserCircle, FaSignInAlt, 
  FaSun, FaMoon, FaUserFriends,
  FaDatabase
} from 'react-icons/fa';
import ClubSearchSelector from './ClubSearchSelector';

const Navbar = ({ theme, toggleTheme }) => {
  const { user, logout } = useAuth(); // Get from Context
  const navigate = useNavigate();
  const location = useLocation();

  const handleLogout = () => {
    logout();
    navigate('/login'); 
  };

  const isActive = (path) => location.pathname === path ? 'var(--accent-primary)' : 'var(--text-muted)';

  const isMember = user && ['clubMember', 'contentManager', 'clubAdmin'].includes(user.role);

  return (
    <nav className="navbar">
      <Link to="/" className="nav-brand"><FaFilm /> FilmClubs<span className="brand-suffix">DB</span></Link>
      
      <div className="nav-menu">
        <Link to="/" className="nav-link" style={{color: isActive('/')}}><FaCalendarAlt /> Feed</Link>
        <div className="nav-center"> <ClubSearchSelector /> </div>
        {!user ? (
          <>
             <button onClick={toggleTheme} className="btn-ghost" title="Switch Theme">
              {theme === 'dark' ? <FaSun size={18} color="#fbbf24" /> : <FaMoon size={18} color="#4b5563" />}
            </button>
            <Link to="/login" className="btn-primary" style={{padding: '8px 16px'}}><FaSignInAlt /> Login</Link>
          </>
        ) : (
          <>

            {['dbAdministrator'].includes(user.role) && (
              <Link to="/system-admin" className="nav-link" style={{color: isActive('/system-admin')}}><FaDatabase /> Admin</Link>
            )}
            {isMember && (
              <Link to="/my-team" className="nav-link" style={{color: isActive('/my-team')}}><FaUserFriends /> My Team</Link>
            )}

            {['contentManager', 'clubAdmin', 'dbAdministrator'].includes(user.role) && (
              <Link to="/content-manager" className="nav-link" style={{color: isActive('/content-manager')}}><FaFilm /> Manage</Link>
            )}
            {['equipmentManager', 'clubAdmin', 'dbAdministrator'].includes(user.role) && (
              <Link to="/manage-equipment" className="nav-link" style={{color: isActive('/manage-equipment')}}><FaTools /> Equipment</Link>
            )}
             {['clubAdmin', 'dbAdministrator'].includes(user.role) && (
              <Link to="/manage-members" className="nav-link" style={{color: isActive('/manage-members')}}><FaUsers /> Members</Link>
            )}

            <div className="user-badge"><FaUserCircle /><span>{user.username}</span></div>
            <button onClick={toggleTheme} className="btn-ghost" title="Switch Theme">
              {theme === 'dark' ? <FaSun size={18} color="#fbbf24" /> : <FaMoon size={18} color="#4b5563" />}
            </button>
            <button onClick={handleLogout} className="btn-ghost" title="Logout"><FaSignOutAlt /></button>
          </>
        )}
      </div>
    </nav>
  );
};
export default Navbar;